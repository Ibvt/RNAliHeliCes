...
typedef Hash::Ref<std::pair<Rope, mfeanswer_v4> , hbpp_insp_hash_h > hbpp_hash_h;
...
hbpp_hash_h LBRope_firstG_mfeanswer_v4_secondG_EMhdecl0x1fdd2e0_zero;
...
void clear() { tabulated.clear(); }
hbpp_hash_h &  get(unsigned int t_0_i, unsigned int t_0_j)
{
  assert( (t_0_i <= t_0_j));
  assert( (t_0_j <= t_0_n));
  if (((t_0_j - t_0_i) < 7))
...
void set(unsigned int t_0_i, unsigned int t_0_j,  const hbpp_hash_h &  e)
{
  assert( (t_0_i <= t_0_j));
  assert( (t_0_j <= t_0_n));
  if (((t_0_j - t_0_i) < 7))
...
void clear() { tabulated.clear(); }
hbpp_hash_h &  get(unsigned int t_0_i)
{
  unsigned int t_0_j = t_0_n;
  assert( (t_0_i <= t_0_j));
  assert( (t_0_j <= t_0_n));
...
void set(unsigned int t_0_i,  const hbpp_hash_h &  e)
{
  unsigned int t_0_j = t_0_n;
  assert( (t_0_i <= t_0_j));
  assert( (t_0_j <= t_0_n));
...
empty(LBRope_firstG_mfeanswer_v4_secondG_EMhdecl0x1fdd2e0_zero);
empty(BRope_firstG_mfeanswer_v4_secondG_E_zero);

t_0_left_most = 0;
t_0_right_most = t_0_seq.size();
hbpp_insp_hash_h::set_k(kbest);
  ...
  private:
    ...
    hbpp_hash_h &  nt_closed(unsigned int t_0_i, unsigned int t_0_j);
    hbpp_hash_h &  nt_dangle(unsigned int t_0_i, unsigned int t_0_j);
    std::pair<Rope, mfeanswer_v4>  nt_hairpin(unsigned int t_0_i, unsigned int t_0_j);
    hbpp_hash_h nt_iloop(unsigned int t_0_i, unsigned int t_0_j);
    hbpp_hash_h nt_initstem(unsigned int t_0_i, unsigned int t_0_j);
    hbpp_hash_h nt_leftB(unsigned int t_0_i, unsigned int t_0_j);
    hbpp_hash_h &  nt_ml_comps(unsigned int t_0_i, unsigned int t_0_j);
    hbpp_hash_h &  nt_ml_comps1(unsigned int t_0_i, unsigned int t_0_j);
    hbpp_hash_h nt_multiloop(unsigned int t_0_i, unsigned int t_0_j);
    hbpp_hash_h nt_rightB(unsigned int t_0_i, unsigned int t_0_j);
    hbpp_hash_h nt_stack(unsigned int t_0_i, unsigned int t_0_j);
    hbpp_hash_h &  nt_struct(unsigned int t_0_i);
...
template <typename Iterator>
    hbpp_hash_h h(std::pair<Iterator, Iterator> i)
;
...
{
  intrusive_ptr<Backtrace_nt_struct_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_struct_Front(intrusive_ptr<Backtrace_nt_struct_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
...
{
  intrusive_ptr<Backtrace_nt_dangle_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_dangle_Front(intrusive_ptr<Backtrace_nt_dangle_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
  return t->backtrack(this->score());
...
{
  intrusive_ptr<Backtrace_nt_initstem_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_initstem_Front(intrusive_ptr<Backtrace_nt_initstem_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
  return t->backtrack(this->score());
...
{
  intrusive_ptr<Backtrace_nt_closed_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_closed_Front(intrusive_ptr<Backtrace_nt_closed_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
  return t->backtrack(this->score());
...
{
  intrusive_ptr<Backtrace_nt_stack_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_stack_Front(intrusive_ptr<Backtrace_nt_stack_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
  return t->backtrack(this->score());
...
{
  intrusive_ptr<Backtrace_nt_hairpin_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_hairpin_Front(intrusive_ptr<Backtrace_nt_hairpin_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
  return t->backtrack(this->score());
...
{
  intrusive_ptr<Backtrace_nt_leftB_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_leftB_Front(intrusive_ptr<Backtrace_nt_leftB_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
  return t->backtrack(this->score());
...
{
  intrusive_ptr<Backtrace_nt_rightB_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_rightB_Front(intrusive_ptr<Backtrace_nt_rightB_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
  return t->backtrack(this->score());
...
{
  intrusive_ptr<Backtrace_nt_iloop_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_iloop_Front(intrusive_ptr<Backtrace_nt_iloop_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
  return t->backtrack(this->score());
...
{
  intrusive_ptr<Backtrace_nt_multiloop_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_multiloop_Front(intrusive_ptr<Backtrace_nt_multiloop_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
  return t->backtrack(this->score());
...
{
  intrusive_ptr<Backtrace_nt_ml_comps_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_ml_comps_Front(intrusive_ptr<Backtrace_nt_ml_comps_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
  return t->backtrack(this->score());
...
{
  intrusive_ptr<Backtrace_nt_ml_comps1_Back<hbpp, Value, pos_int> > back;

Backtrace_nt_ml_comps1_Front(intrusive_ptr<Backtrace_nt_ml_comps1_Back<hbpp, Value, pos_int> > b) : back(b) {}
intrusive_ptr<Backtrace<Value, pos_int> > backtrack()
{
  intrusive_ptr<Backtrace_NT_Back_Base<std::pair<Rope, mfeanswer_v4> , hbpp, Value, pos_int> > t = back;
  return t->backtrack(this->score());
...
intrusive_ptr<Eval_List<Value> > eval()
{
  ...
  return t->eval();
    ...
    List_Ref<std::pair<std::pair<Rope, mfeanswer_v4> , intrusive_ptr<Backtrace<String, unsigned int> > > > h_bt(List_Ref<std::pair<std::pair<Rope, mfeanswer_v4> , intrusive_ptr<Backtrace<String, unsigned int> > > > i, hbpp_hash_h &  left_answers);
template <typename Iterator>
    List_Ref<std::pair<std::pair<Rope, mfeanswer_v4> , intrusive_ptr<Backtrace<String, unsigned int> > > > h_bt(std::pair<Iterator, Iterator> i, hbpp_hash_h &  left_answers)
;
...
namespace gapc {
  typedef hbpp hishapeb_pp_cls;
  typedef hbpp_hash_h &  hishapeb_pp_ret;
}
#endif
