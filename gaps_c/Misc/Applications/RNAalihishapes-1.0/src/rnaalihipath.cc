...
//   }
  ...
  hishapes=MallocStringArray(100,HISHREPSIZE);
...
//	strcpy(hishapes[n_struc], buf+length+4);
  ...
  seq = (mis) ? consens_mis((const char **) AS) : consensus((const char **) AS);
  //printf("Consensus sequence: %s\nStart structure: %s\nTarget structure: %s\n", seq, struc1, struc2);    // seq is consensus sequence
  //printf("Consensus sequence: %s\nStart structure: %s\nTarget structure: %s\n", seq, structures[0], structures[1]);    // seq is consensus sequence
  //printf("%s\n%s\n", seq, structures[1]);
...
//#### for path calculation #### 
	...
	printf("Consensus sequence: %d\nStart structure: %d\nTarget structure: %d\n", strlen(seq), strlen(struc1), strlen(struc2));    // seq is consensus sequence
