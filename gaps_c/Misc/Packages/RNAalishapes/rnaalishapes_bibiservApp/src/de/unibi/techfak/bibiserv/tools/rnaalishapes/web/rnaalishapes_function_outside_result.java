...
/**
 * This is a autogenerated bean result template class for function <i>rnaalishapes_function_outside_result</i>
 *
 ...
 */
public class rnaalishapes_function_outside_result implements ToolFunctionResult {

    Logger log = Logger.getLogger(rnaalishapes_function_outside_result.class);
    ...
    public List<Pair<String,Pair<String,String>>> getAllOutputFiles(){
        ...
        int counter;
        	...
        	counter=0;
	for(String filenames: execfunction.getMatchingFiles("","dotPlot.ps")) {
counter++;
		tmp = new Pair<String,String>(filenames, "application/postscript");
		files.add(new Pair<String,Pair<String,String>>(messages.property("rnaalishapes_additionaloutput_dotplot_name")+ ((counter>1) ? " "+counter : "") , tmp));
}
    ...
    public boolean applyVisualizer(OntoRepresentation type, String visualizer){
        try {
            ...
            FacesContext.getCurrentInstance().getExternalContext().redirect("/rnaalishapes?viewType=submission&subType=rnaalishapes_function_outside_visualization");
        } catch (Exception e){
            log.fatal(e.getMessage(),e);
            return false;
        }
