...
PUBLIC int gmlRNA(char *string, char *structure, char *ssfile, char option)
{
  ...
  fprintf(gmlfile,
          ...
          "# CreationDate: %s\n"
          "# Name: %s\n"
          "# Options: %s\n", PACKAGE_VERSION, time_stamp(), ssfile, option_string());
  fprintf(gmlfile,
          "graph [\n"
          " directed 0\n");
  for (i=1; i<=length; i++){
...
int PS_rna_plot_a(char *string, char *structure, char *ssfile, char *pre, char *post)
{
  ...
  fprintf(xyplot,
          ...
          "%%%%Pages: 1\n"
          "%%%%EndComments\n\n"
          "%%Options: %s\n", rcsid+5, PACKAGE_VERSION, time_stamp(), option_string());
...
{           /* produce input for XRNA RNA drawing program */
  ...
  fprintf(ss_file,
          ...
          "# CreationDate: %s\n"
          "# Options: %s\n", PACKAGE_VERSION, time_stamp(), option_string());
  for (i=1; i<=length; i++)
...
static FILE * PS_dot_common(char *seq, char *wastlfile,
  ...
  fprintf(wastl,
          ...
          "%%%%Title: RNA Dot Plot\n"
          "%%%%Creator: %s, ViennaRNA-%s\n"
          "%%%%CreationDate: %s", rcsid+5, PACKAGE_VERSION, time_stamp());
  if (winsize>0)
