...
class Pairs {
	...
	public:
		...
		void setStructure(std::pair<const char*, unsigned int> structure) {
			...
			for (unsigned int i = 0; i < structure.second; i++) {
				...
				if (getOpenType(structure.first[i]) != '-') { // position is a closing base
					...
					//printf("(%d, %d)\t", stacks.at(getOpenType(structure.first[i])).back(), i);
		...
		void setStructure(const short* pt) {
		        basepairs.clear();
		        unsigned len = pt[0];
			for (unsigned i=1; i<=len; i++) {
			  if (i < pt[i]) {
			    //printf("(%d, %d)\t", i-1, pt[i]-1);
			    basepairs.insert(std::pair<unsigned int, unsigned int>(i-1, pt[i]-1));
			  }
			}
		}
...
inline bool basepair_eval(const Basic_Sequence<alphabet, pos_type> &seq, T i, T j) {
...
inline bool regionpair_eval(int i, int j, int len) {
	int k;
	for (k = 0; k < len; k++) {
...
template<typename alphabet, typename pos_type, typename T>
inline bool unpaired_eval(const Basic_Sequence<alphabet, pos_type> &seq, T i, T j) {
